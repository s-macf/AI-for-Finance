...
load_daily_returns <- function(asset_symbols, startDate, endDate) {
  ...
  assets_train <- lapply(asset_symbols, function(sym) {
    tryCatch(
      ...
      error = function(e) {
        removed_assets <<- append(removed_assets, sym); NULL
      }
    )
  })
...
train_test_split <- function(asset, seq_length, target_feature, test_size = 0.2) {
  ...
  x_train <- x_train[, , setdiff(1:num_features, target_feature), drop = FALSE]
  x_test  <- x_test[, , setdiff(1:num_features, target_feature), drop = FALSE]
...
target_feature = 4
split_data <- train_test_split(features, seq_length, target_feature)
